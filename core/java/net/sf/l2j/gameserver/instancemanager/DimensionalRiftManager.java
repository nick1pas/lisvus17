...
public class DimensionalRiftManager
{
    ...
    private void loadRooms()
    {
            ...
            ResultSet rs = s.executeQuery())
        {
            while (rs.next())
            {
                ...
                byte roomId = rs.getByte("room_id");
                ...
                _rooms.get(type).put(roomId, new DimensionalRiftRoom(roomId, xMin, xMax, yMin, yMax, z1, z2, xT, yT, zT, isBossRoom));
            }
        }
        catch (Exception e)
        {
        ...
        Set<Byte> roomIds = _rooms.keySet();
        int typeSize = roomIds.size();
        int roomSize = 0;
                                ...
                                if ("room".equalsIgnoreCase(room.getNodeName()))
                                {
                                    ...
                                    for (Node spawn = room.getFirstChild(); spawn != null; spawn = spawn.getNextSibling())
                                    {
                                        if ("spawn".equalsIgnoreCase(spawn.getNodeName()))
                                        {
                                            ...
                                            {
                                                riftRoom = _rooms.get(type).get(roomId);
                                            }
                                            
                                            for (int i = 0; i < count; i++)
                                            {
                                                if (template != null && riftRoom != null)
                                                {
    ...
    public void reload()
    {
        ...
        {
            Map<Byte, DimensionalRiftRoom> roomData = _rooms.get(b);
            for (byte i : roomData.keySet())
            {
                roomData.get(i).getSpawns().clear();
            }
            roomData.clear();
        }
        _rooms.clear();
    ...
    {
        Map<Byte, DimensionalRiftRoom> roomData = _rooms.get((byte) 0);
        return ignorePeaceZone ? roomData.get((byte) 1).checkIfInZone(x, y, z) : roomData.get((byte) 1).checkIfInZone(x, y, z) && !roomData.get((byte) 0).checkIfInZone(x, y, z);
    }
    ...
    public synchronized void start(L2PcInstance player, byte type, L2NpcInstance npc)
    {
        ...
        Map<Byte, DimensionalRiftRoom> roomData = _rooms.get(type);

        do
        {
            emptyRooms = getFreeRooms(type);
            roomId = emptyRooms.get(Rnd.get(1, emptyRooms.size()) - 1);
        }
        // find empty room
        while (roomData.get(roomId).isPartyInside());
    ...
    public boolean isAllowedEnter(byte type)
    {
        ...
        for (Object room : roomData.values())
        {
            if (((DimensionalRiftRoom) room).isPartyInside())
                count++;
        }
        return (count < (roomData.size() - 1));
    }
