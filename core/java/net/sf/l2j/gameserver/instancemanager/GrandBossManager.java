    ...
    private void storeToDb()
    {
        try (Connection con = L2DatabaseFactory.getInstance().getConnection())
        {
            ...
            for (L2BossZone zone : _zones.values())
            {
                ...
                {
                    for (Integer playerId : allowedPlayers)
                    {
        ...
        try (Connection con = L2DatabaseFactory.getInstance().getConnection();
            PreparedStatement st1 = con.prepareStatement("UPDATE grandboss_data SET status = ? WHERE boss_id = ?");
            PreparedStatement st2 = con.prepareStatement("UPDATE grandboss_data SET loc_x = ?, loc_y = ?, loc_z = ?, heading = ?, respawn_time = ?, currentHP = ?, currentMP = ?, status = ? WHERE boss_id = ?"))
        {
            for (Integer bossId : _storedInfo.keySet())
            {
                ...
                {
                    st1.setInt(1, _bossStatus.get(bossId));
                    st1.setInt(2, bossId);
                    st1.addBatch();
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        mp = boss.getCurrentMp();
                    }

                    st2.setInt(1, boss.getX());
                    st2.setInt(2, boss.getY());
                    st2.setInt(3, boss.getZ());
                    st2.setInt(4, boss.getHeading());
                    st2.setLong(5, info.getLong("respawn_time"));
                    st2.setDouble(6, hp);
                    st2.setDouble(7, mp);
                    st2.setInt(8, _bossStatus.get(bossId));
                    st2.setInt(9, bossId);
                    st2.addBatch();
                }
            }
