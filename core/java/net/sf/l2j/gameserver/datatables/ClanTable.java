...
public class ClanTable
{
    ...
    public L2Clan createClan(L2PcInstance player, String clanName)
    {
        ...
        L2ClanMember leader = new L2ClanMember(clan, player.getName(), player.getLevel(), player.getClassId().getId(), player.getAppearance().getSex(), player.getRace().ordinal(), player.getPowerGrade(), player.getObjectId());
    ...
    public void destroyClan(L2Clan clan)
    {
        ...
        try (Connection con = L2DatabaseFactory.getInstance().getConnection())
        {
            try (PreparedStatement statement = con.prepareStatement("UPDATE characters SET clanid = 0, power_grade = 0 WHERE clanid=?"))
            {
