...
public abstract class L2Skill
{
	...
	public final L2Object[] getTargetList(L2Character activeChar, boolean onlyFirst)
	{
		...
		final L2Character target = objTarget instanceof L2Character ? (L2Character) objTarget : null;
		...
		switch (targetType)
		{
			...
			case TARGET_OWNER_PET:
			{
				...
				{
					L2PcInstance owner = ((L2Summon) activeChar).getOwner();
			...
			{
				boolean hasCastRange = getCastRange() >= 0;
				...
				{
					boolean isTargetAttackableOrPlayable = (target instanceof L2Attackable || target instanceof L2PlayableInstance);
					
					if (hasCastRange && (target == activeChar || !isTargetAttackableOrPlayable || target.isDead()))
					{
						activeChar.sendPacket(new SystemMessage(SystemMessage.TARGET_IS_INCORRECT));
						return null;
					}
				...
				for (L2Character obj : objs)
				{
					...
					if (Util.checkIfInRange(radius, origin, obj, true))
					{
						...
						if (!checkForAreaOffensiveSkills(activeChar, origin == activeChar ? null : origin, obj, srcInArena))
						{
							continue;
						}
			...
			case TARGET_ALLY:
			{
				if (activeChar instanceof L2PcInstance)
				{
					...
					if (clan != null)
					{
						// Get all visible objects in a spheric area near the L2Character
						// Get Clan Members
						for (L2Character knownCharacter : activeChar.getKnownList().getKnownCharactersInRadius(radius))
						{
							if (!(knownCharacter instanceof L2PcInstance))
							{
								continue;
							}

							L2PcInstance newTarget = (L2PcInstance) knownCharacter;
							...
							L2Summon newTargetSummon = newTarget.getPet();
							if (newTargetSummon != null)
							{
								if (Util.checkIfInRange(radius, activeChar, newTargetSummon, true))
								{
									if (targetType != SkillTargetType.TARGET_CORPSE_ALLY && !newTargetSummon.isDead() && !onlyFirst)
									{
										targetList.add(newTargetSummon);
									}
								}
							}
					...
					if (clan != null)
					{
						...
						for (L2ClanMember member : clan.getMembers())
						{
							...
							if (newTargetSummon != null)
							{
								if (Util.checkIfInRange(radius, activeChar, newTargetSummon, true))
								{
									if (targetType != SkillTargetType.TARGET_CORPSE_CLAN && !newTargetSummon.isDead() && !onlyFirst)
									{
										targetList.add(newTargetSummon);
									}
								}
	...
	public final boolean checkForAreaOffensiveSkills(L2Character caster, L2Character originTarget, L2Character target, boolean sourceInArena)
	{
		...
		if (player != null)
		{
			...
			if (targetPlayer != null)
			{
				...
				if (isOffensive() && player.getSiegeState() > 0 && player.isInsideZone(L2Character.ZONE_SIEGE) && player.getSiegeState() == targetPlayer.getSiegeState() && player.getSiegeSide() == targetPlayer.getSiegeSide())
				{
					return false;
				}
		...
		else
		{
			if (target instanceof L2Attackable)
			{
				...
				{
					if (!caster.isConfused() && (!caster.isCoreAIDisabled() || ((L2Attackable) caster).getFactionId() != null && ((L2Attackable) caster).getFactionId().equals(((L2Attackable) target).getFactionId())))
					{
						return false;
					}
				}
