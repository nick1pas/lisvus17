...
public class DimensionalRift
{
    ...
    private final List<Byte> _completedRooms = new ArrayList<>();
    private final List<L2PcInstance> _deadPlayers = new CopyOnWriteArrayList<>();
    ...
    protected void createTeleporterTimer()
    {
        ...
        _teleporterTimerTask = new TimerTask()
        {
            ...
            public void run()
            {
                ...
                if (_currentJumps < getMaxJumps() && _party.getMemberCount() > 1 && _party.getMemberCount() > _deadPlayers.size())
                {
                    _currentJumps++;
    ...
    {
        return _teleporterTimer;
    }
    
    public TimerTask getTeleportTimerTask()
    {
        return _teleporterTimerTask;
    }
    ...
    {
        _isBossRoom = DimensionalRiftManager.getInstance().getRoom(_type, room).isBossRoom();
    }
