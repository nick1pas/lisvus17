...
public class L2SignsPriestInstance extends L2FolkInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (command.startsWith("sevensigns"))
		{
			...
			int stoneType = 0;
			L2ItemInstance ancientAdena = player.getInventory().getItemByItemId(Inventory.ANCIENT_ADENA_ID);
			...
			switch (val)
			{
				...
				case 20: // Seal Status (for when joining a cabal)
					StringBuilder contentBuffer = StringUtil.startAppend(200, "<html><body><font color=\"LEVEL\">[ Seal Status ]</font><br>");
					...
					for (int i = 1; i < 4; i++)
					{
						...
						{
							StringUtil.append(contentBuffer, "[" + SevenSigns.getSealName(i, false) + ": " + SevenSigns.getCabalName(sealOwner) + "]<br>");
						}
						else
						{
							StringUtil.append(contentBuffer, "[" + SevenSigns.getSealName(i, false) + ": Nothingness]<br>");
						}
					}
					
					StringUtil.append(contentBuffer, "<a action=\"bypass -h npc_" + getObjectId() + "_SevenSigns 3 " + cabal + "\">Go back.</a></body></html>");
