...
/**
 * This class handles following admin commands: - edit_character - current_player - character_list - character_info - show_characters - find_character - find_ip - find_account - rec - nokarma - setkarma - settitle - setname - setsex - setclass - fullfood - save_modifications
...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	private static String[] _adminCommands =
	{
		...
		"admin_setcolor", // change char name color
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_character_list") || command.startsWith("admin_character_info") || command.startsWith("admin_debug"))
		{
			try
			{
			...
			{
				activeChar.sendMessage("Usage: //<command> <player_name>");
			}
		}
		...
		else if (command.startsWith("admin_setclass"))
		{
			try
			{
				...
				{
					showHelpPage(activeChar, "charclasses.htm");
					return true;
				}
		...
		else if (command.startsWith("admin_setcolor"))
		{
			try
			{
				...
				try (Connection con = L2DatabaseFactory.getInstance().getConnection();
					PreparedStatement statement = con.prepareStatement( "UPDATE characters SET name_color=? WHERE obj_Id=?"))
				{
					statement.setInt(1, player.getAppearance().getNameColor());
					statement.setInt(2, player.getObjectId());
					statement.execute();
				}
				catch (Exception e)
				{
					_log.warning("Could not update char name color: " + e);
				}
			}
			catch (Exception e)
			{
	...
	private void listCharacters(L2PcInstance activeChar, int page)
	{
		...
		{
			int pagenr = x + 1;
			StringUtil.append(replyMSG, "<center><a action=\"bypass -h admin_show_characters " + x + "\">Page " + pagenr + "</a></center>");
		}
		...
		for (int i = charactersStart; i < charactersEnd; i++)
		{
			...
			StringUtil.append(replyMSG, "<tr><td width=80><a action=\"bypass -h admin_character_info " + players[i].getName() + "\">" + players[i].getName() + "</a></td><td width=110>" + players[i].getTemplate().className + "</td><td width=40>" + players[i].getLevel() + "</td></tr>");
		}
	...
	private void gatherCharacterInfo(L2PcInstance activeChar, L2PcInstance player, String filename)
	{
		...
		adminReply.replace("%runspeed%", String.valueOf(player.getMoveSpeed()));
	...
	private void adminModifyCharacter(L2PcInstance activeChar, String modifications)
	{
		...
		showCharacterInfo(activeChar, player); // Back to start
	}
	...
	private void findCharacter(L2PcInstance activeChar, String CharacterToFind)
	{
		...
		for (L2PcInstance player : players)
		{
			...
			if (name.toLowerCase().contains(CharacterToFind.toLowerCase()))
			{
				...
				StringUtil.append(replyMSG, "<tr><td width=80><a action=\"bypass -h admin_character_list " + name + "\">" + name + "</a></td><td width=110>" + player.getTemplate().className + "</td><td width=40>" + player.getLevel() + "</td></tr>");
			}
		...
		{
			StringUtil.append(replyMSG, "s. Please try again.");
		}
		else if (CharactersFound > 20)
		{
			adminReply.replace("%number%", " more than 20");
			StringUtil.append(replyMSG, "s.<br>Please refine your search to see all of the results.");
		}
	...
	private void findCharactersPerIp(L2PcInstance activeChar, String IpAdress) throws IllegalArgumentException
	{
		...
		if (CharactersFound == 0)
		{
			StringUtil.append(replyMSG, "s. Maybe they got d/c? :)");
		}
		else if (CharactersFound > 20)
		{
			adminReply.replace("%number%", " more than " + String.valueOf(CharactersFound));
			StringUtil.append(replyMSG, "s.<br>In order to avoid you a client crash I won't <br1>display results beyond the 20th character.");
		}
		else if (CharactersFound == 1)
		{
			StringUtil.append(replyMSG, ".");
	...
	{
		if (Util.isValidName(characterName, Config.CHAR_NAME_TEMPLATE))
		{
			String account = null;
		...
		{
			activeChar.sendMessage("The new name does not natch with expression pattern.");
		}
	}
